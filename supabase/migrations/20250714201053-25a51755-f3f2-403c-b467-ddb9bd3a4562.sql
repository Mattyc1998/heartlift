-- Add unique constraint with proper handling of null coach_id
ALTER TABLE public.user_usage DROP CONSTRAINT IF EXISTS user_usage_user_id_date_coach_id_key;

-- Create a unique index that treats null coach_id values as distinct
CREATE UNIQUE INDEX user_usage_unique_user_date_coach 
ON public.user_usage (user_id, date, COALESCE(coach_id, ''));

-- Update the increment function to handle the constraint properly
