...
serve(async (req) => {
  ...
  try {
    ...
    // Create a new client with the user's access token for proper auth context
    ...
    // Removed hardcoded premium access for security - all users must have valid subscriptions
    ...
    let clientError = 'An error occurred checking subscription status.';
    let statusCode = 500;
    
    if (errorMessage.includes('authentication') || errorMessage.includes('authorized')) {
