            ...
            <TabsContent value="current" className="space-y-4">
              {currentReport && !isLoading ? (
                <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  <Card className="lg:col-span-2 bg-gradient-to-br from-primary/5 via-primary/3 to-background border-primary/20">
                    <CardHeader className="pb-3 p-4 sm:p-6">
                      <CardTitle className="text-base sm:text-lg flex items-center gap-2">
                    ...
                    <CardContent className="p-4 sm:p-6 pt-0">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col items-center p-4 rounded-xl bg-background/50 backdrop-blur-sm border">
                          <Calendar className="w-6 h-6 text-primary mb-2" />
                          <span className="text-2xl font-bold text-primary">{currentReport.conversation_count}</span>
                          <span className="text-xs text-muted-foreground mt-1">Conversations Analysed</span>
                        </div>
                        <div className="flex flex-col items-center p-4 rounded-xl bg-background/50 backdrop-blur-sm border">
                          <Heart className="w-6 h-6 text-pink-500 mb-2" />
                          <span className="text-2xl font-bold text-pink-500">{currentReport.mood_entries_analyzed}</span>
                          <span className="text-xs text-muted-foreground mt-1">Mood Entries</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              {pastReports && pastReports.length > 0 ? (
                <div className="space-y-4">
                  {pastReports.map((report) => (
                    <Card key={report.id} className="bg-gradient-to-br from-primary/5 to-background hover:shadow-lg transition-shadow border-primary/10">
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <CardTitle className="text-lg flex items-center gap-2">
                                <Brain className="w-5 h-5 text-primary" />
                                Report from {new Date(report.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                              </CardTitle>
                              <CardDescription className="flex flex-wrap items-center gap-3 mt-3">
                                <span className="flex items-center gap-1.5 px-2 py-1 bg-background/50 rounded-lg">
                                  <Clock className="w-3 h-3 text-blue-500" />
                                  <span className="text-xs">{report.conversation_count} chats</span>
                                </span>
                                <span className="flex items-center gap-1.5 px-2 py-1 bg-background/50 rounded-lg">
                                  <Heart className="w-3 h-3 text-pink-500" />
                                  <span className="text-xs">{report.mood_entries_analyzed} moods</span>
                                </span>
                                <Badge variant="outline" className="capitalize">{report.attachment_style}</Badge>
                              </CardDescription>
                            </div>
                          </div>
                          
                          <div className="space-y-3 p-3 bg-background/30 rounded-xl">
                            <div className="flex items-center gap-2">
                              <TrendingUp className="w-4 h-4 text-green-500" />
                              <span className="text-sm font-medium">
                                Progress: {report.healing_progress_score}%
                              </span>
                            </div>
