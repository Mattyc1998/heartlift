...
async def sync_subscription_status(request: SubscriptionSyncRequest):
    """
    Sync subscription status from RevenueCat
    """
    try:
        logger.info(f"Syncing subscription for user {request.user_id}")
        
        # Get expiration date if premium is active
        premium_expires_at = None
        if request.has_premium and request.customer_info.get('entitlements', {}).get('active', {}).get('premium'):
            premium_entitlement = request.customer_info['entitlements']['active']['premium']
            premium_expires_at = premium_entitlement.get('expiresDate')
        ...
        await db.subscriptions.update_one(
            ...
            {"$set": {
                "user_id": request.user_id,
                "has_premium": request.has_premium,
                "has_healing_kit": request.has_healing_kit,
        ...
        logger.info(f"Subscription synced successfully for user {request.user_id}")
...
async def get_subscription_status(user_id: str):
    ...
    try:
        ...
        return SubscriptionStatusResponse(
            has_premium=subscription.get("has_premium", False),
            has_healing_kit=subscription.get("has_healing_kit", False),
